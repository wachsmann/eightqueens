...
public class JavaApplication1 {
    ...
    private static Boolean bruteForce(int[] queens, int row) {

        System.out.println("searching...");
        return heap(queens, row);

    }

    private static Boolean heap(int[] queens, int row) {
        int n = queens.length;
        ...
        while (i < n) {
            if (indexes[i] < i) {
                if (indexes[i] != row && i != row) {
                    swap(queens, i % 2 == 0 ? 0 : indexes[i], i);
                }
        ...
        while (!found) {
            for (int i = 0; i < queens.length - 1; i++) {
                ...
                for (int j = 1; j < queens.length; j++) {
                    ...
                    for (int k = queens.length - j; k >= 1; k--) {
                        if (j != row && k != row) {
